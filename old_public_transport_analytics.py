...
def init_routes(app):
    ...
    def fill_transport_report_template():
        ...
        try:
            ...
            # 详细记录接收到的数据结构
            app.logger.info("="*50)
            app.logger.info("接收到前端请求数据 - 开始详细记录")
            app.logger.info(f"数据类型: {type(data)}")
            app.logger.info(f"包含的字段: {list(data.keys())}")
            ...
            conclusion = data.get('conclusion', {})  # 获取结论数据
            
            # 记录站点数据
            app.logger.info(f"站点数据类型: {type(stations)}, 站点数量: {len(stations)}")
            if stations:
                app.logger.info("第一个站点数据详情:")
                first_station = stations[0]
                app.logger.info(f"数据类型: {type(first_station)}")
                app.logger.info(f"字段列表: {list(first_station.keys())}")
                app.logger.info(f"原始数据: {json.dumps(first_station, ensure_ascii=False)}")
                
                # 检查关键字段
                for key in ['index', 'name', 'type', 'distance', 'detail', 'location']:
                    if key in first_station:
                        value = first_station[key]
                        app.logger.info(f"字段 '{key}': 值={value}, 类型={type(value).__name__}")
                    else:
                        app.logger.info(f"字段 '{key}': 不存在")
                
                # 检查所有站点的类型字段
                app.logger.info("所有站点的type字段值:")
                for i, station in enumerate(stations):
                    app.logger.info(f"站点 {i+1} type: {station.get('type', '没有type字段')}")
            
            app.logger.info("接收到前端请求数据 - 详细记录结束")
            app.logger.info("="*50)
            
            # 记录接收到的站点数据结构
            app.logger.info(f"接收到站点数据数量: {len(stations)}")
            ...
            try:
                ...
                if conclusion:
                    ...
                    app.logger.info(f"添加结论信息: {conclusion_text}")
                    
                    # 确保结论占位符被识别 - 直接添加到all_placeholders
                    if '结论' not in all_placeholders:
                        all_placeholders.append('结论')
                        app.logger.info("手动添加'结论'到占位符列表")
...
')
                    ...
                    # 如果文本已更改且不是结论占位符的特殊处理，应用新文本
                    if text_changed and not ('{结论}' in original_text and '结论' in extended_project_info):
                    ...
                    if '{地图截图}' in paragraph.text and temp_image_path:
                        ...
                        paragraph.alignment = WD_ALIGN_PARAGRAPH.CENTER
                        app.logger.info(f"替换了地图截图占位符（无图片）")    # 处理公交站点列表占位符
